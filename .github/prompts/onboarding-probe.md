...
4. Map the **ordered** path a literal doc-follower would take: which pages, in what order, from install → API key → first snapshot inference (software-only path).
...
**Rule 5: Track wall-clock time** from when you start following the docs to when you achieve a first valid snapshot inference. Record it in seconds.
...
**Success criterion for this path (snapshot / software-only):**
...
**1. Header line**
```
DX Probe — snapshot path | Date: <today> | Platform: __RUNNER_LABEL__ (__IMAGE_OS__ __IMAGE_VERSION__) | Docs: __DOCS_SITE__
```
...
- Time to first snapshot inference: Xs (target: <5 min)
...
- `reached_success`: `true` ONLY if you observed a valid snapshot inference (the success criterion above); `false` otherwise.
