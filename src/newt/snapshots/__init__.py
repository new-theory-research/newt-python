...
Each snapshot is one observation frame extracted from a recorded episode on an NT
...
prompt, `robot.infer(snapshots.load(name))` works with no extra arguments — the
...
# Maps snapshot name -> bundled data file. Camera frames are stored JPEG-compressed
# to keep each snapshot under the package-size budget; load() decodes them back to
...
# tag ("blue cups stitch cup") is too rough for docs; substitute ModelA-FP3's clean
# canonical task prompt, which still matches the obs and returns a non-degenerate
# chunk. Snapshots absent from this map keep their recorded prompt.
_PROMPT_OVERRIDES = {
...
def load(name: str) -> dict:
    ...
    Args:
        name: One of the available snapshots (see `available()`): "cup_stacking",
              "pour_coffee_beans".
    ...
    data_pkg = resources.files(__package__) / "data" / _SNAPSHOTS[name]
