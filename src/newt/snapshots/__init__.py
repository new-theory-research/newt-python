...
Usage:
    ...
    robot = newt.Robot(api_key=..., model="model-a-pour")
    response = robot.infer(snapshots.load("cup_stacking"))
    print(response)  # labeled chunk

Available snapshots:
    cup_stacking       — 8-axis modela rig (left-arm tr3 cup-stacking episode; wrist camera
                         remapped to right-wrist-camera). Cameras: right-wrist-camera,
                         surrounding1, surrounding2.
    pour_coffee_beans  — 8-axis modela rig (native right-arm tr2 pour episode). Same cameras.
...
class _Snapshot:
    ...
    8-axis modela rig has three cameras, a 6-axis SO-101 rig has two (top/side). There is
...
# uint8 CHW arrays (H/W per the rig's contract — 240x320 for modela, 378x378 for so101).
#
# cup_stacking is listed first: it is the historical default `newt run` reaches for when
# a model's contract declares nothing to match on (an modela base with no state_shape/cameras).
_SNAPSHOTS: dict[str, _Snapshot] = {
    "cup_stacking": _Snapshot(
        ...
        # Substitute ModelA-FP3's clean canonical task prompt, which still matches the obs
...
def _decode_jpeg(buf: np.ndarray) -> np.ndarray:
    ...
    The H/W come from the encoded frame itself — never assumed. An modela frame decodes to
...
def load(name: str) -> dict:
    ...
    Returns:
        ...
        D and (H, W) are per-rig (8 / 240x320 for modela, 6 / 224x224 for so101 red_cube). The
...
def describe(name: str) -> dict:
    ...
    Returns:
        {
            "state_shape": tuple[int, ...],   — e.g. (8,) for modela, (6,) for so101,
