...
dimension and camera set MATCH — an 8-axis modela model gets an 8-axis snapshot, a 6-axis
...
# The fallback snapshot when a model's contract declares nothing to match on (an modela base
...
# `newt run <modela-tag>` and gets a real cup-stacking observation without knowing a snapshot
# name exists.
...
def _usage() -> None:
    ...
    print("                     model's contract — 8-axis modela, 6-axis SO-101)")
...
def _positional_tag(args: list[str]) -> str | None:
    ...
    ``newt run model-a-pour --snapshot pour_coffee_beans --json`` → ``model-a-pour``.
    """
    skip_next = False
    for a in args:
        if skip_next:
...
def _select_snapshot(snapshots, contract) -> str | None:
    ...
    8-axis tie over pour_coffee_beans — preserving the historical modela default.

    Two Rule-10 seams:
      - A contract that declares NOTHING to match on (state_shape and cameras both
        absent — an modela base whose entry carries only action_shape, or the empty
