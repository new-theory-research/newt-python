...
# model family). Fine-tune variants (UIDs or tags like "model-a-pour-coffee-beans")
...
# Default (model=None): connects to the "model-a" server; server uses its own
...
_MODEL_ENDPOINTS: dict[str, str] = {
    ...
    "pi05_aloha": "wss://removed-endpoint.invalid/stream",
...
class Robot:
    ...
    def __init__(
        ...
        if env_url:
            ...
            _endpoint_key = model if model is not None else "model-a"
...
def list_models(api_key: str, base_url: str | None = None) -> list[dict]:
    ...
    Args:
        ...
        base_url: Override HTTP base URL (e.g. http://localhost:8000). Defaults to
                  deriving from NT_INFERENCE_URL env var or the default model-a server.
    ...
    if base_url is None:
        env_url = os.environ.get("NT_INFERENCE_URL")
        ws_url = env_url or _MODEL_ENDPOINTS["model-a"]
