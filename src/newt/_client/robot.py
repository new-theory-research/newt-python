...
class ColdStartRetry(UserWarning):
    ...
    Modal containers serving large ModelA-FP3 checkpoints (~5GB) take 60–90s to warm
...
class ModelContract:
    ...
    Attributes:
        state_shape:  Expected proprioceptive state vector shape, e.g. ``(8,)``
                      for model-a, ``(6,)`` for so101. ``None`` if not declared.
...
class Robot:
    ...
    def contract(self) -> ModelContract | None:
        ...
        tell you here," not "this model has no shape."
            ...
            >>> robot = newt.Robot(model="model-a")
    ...
    def _ws_connect(self):
        ...
        try:
            ...
            if self._cold_start_retry_consumed:
                raise
            self._cold_start_retry_consumed = True
            model_str = self._model or "model-a"
            warnings.warn(
                ColdStartRetry(
