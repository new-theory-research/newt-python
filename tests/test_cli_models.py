...
_REAL_7_MODELS = [
    ...
    {
        "uid": "ft_4hn40z6a",
        "tags": ["model-a-pour-coffee-beans", "pour-coffee-beans"],
        "type": "fine_tune",
        "base": "ft_base_modelafp3",
        "contract": {"action_axes": ["x", "y", "z", "qw", "qx", "qy", "qz", "gripper"]},
    },
    {
        "uid": "ft_6d0cfd51_e63fbb",
        "tags": ["model-a-clean-table"],
        "type": "fine_tune",
        "base": "ft_base_modelafp3",
        "contract": {"action_axes": ["x", "y", "z", "qw", "qx", "qy", "qz", "gripper"]},
    },
    {
        ...
        "tags": ["model-a-pour-coffee-beans-wm"],
        "type": "fine_tune",
        "base": "ft_base_modelafp3",
        "contract": {"action_axes": ["x", "y", "z", "qw", "qx", "qy", "qz", "gripper"]},
    },
...
def test_grouped_layout_base_names_lead():
    """Base display names lead each family group — model-a / molmoact2 / pi05_aloha.
    ...
    assert names == ["model-a", "molmoact2", "pi05_aloha"], (
...
def test_axes_appear_once_on_modelafp3_base_only():
    """Action axes appear exactly once: on the model-a base line.
    ...
    # That one line must be the model-a base line
    assert "model-a" in axes_lines[0], (
        f"axes line must be the model-a base, got: {axes_lines[0]!r}"
    )
...
def test_uid_alignment_within_family_with_ids():
    ...
    output = _render_models(_REAL_7_MODELS, show_ids=True)
    lines = output.splitlines()
    # Get the model-a family's fine-tune lines
    modela_fts = []
    in_modela = False
    for line in lines:
        if line and not line.startswith("    "):
            in_modela = "model-a" in line
        elif in_modela and line.startswith("    "):
            modela_fts.append(line)

    # Find the position where uid starts on each line (after padding)
    uid_positions = []
    for line in modela_fts:
...
def test_orphan_renders_at_top_level_before_families():
    ...
    base_idx = next(i for i, l in enumerate(non_empty) if "model-a" in l)
...
def test_models_renders_catalog_with_ids(monkeypatch):
    ...
    assert exit_code == 0, f"expected exit 0, got {exit_code}; stderr={err!r}"
    assert "ft_base_modelafp3" in out
...
def test_models_renders_axes_from_contract(monkeypatch):
    ...
    modela_base_line = next(l for l in lines if "model-a" in l and not l.startswith("    "))

    assert "axes" in modela_base_line, f"axes must render on the model-a base line: {modela_base_line!r}"
    assert "gripper" in modela_base_line, f"axis labels must appear: {modela_base_line!r}"
...
def test_models_json_flag_emits_valid_array(monkeypatch):
    ...
    assert "ft_base_modelafp3" in uids
