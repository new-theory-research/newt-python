...
_ModelA_AXES = ["x", "y", "z", "qw", "qx", "qy", "qz", "gripper"]
...
_FAKE_REGISTRY = [
    {
        ...
        "contract": {"action_shape": [50, 8], "action_axes": _ModelA_AXES},
    },
    {
        "uid": "ft_4hn40z6a",
        "tags": ["clean_table"],
...
def test_repr_shows_shape_labels_and_latency():
    ...
    resp = InferenceResponse(
        np.zeros((50, 8), dtype=np.float32), _ModelA_AXES, 261.0, model="model-a"
    )
    assert repr(resp) == (
...
def test_action_chunk_is_canonical_no_regression():
    ...
    resp = InferenceResponse(chunk, _ModelA_AXES, 12.0)
...
def test_registry_resolves_labels_for_base_tag_and_finetune():
    ...
    assert _resolve_action_axes(_FAKE_REGISTRY, None) == _ModelA_AXES  # default UID
    assert _resolve_action_axes(_FAKE_REGISTRY, "model-a") == _ModelA_AXES  # base tag
    assert _resolve_action_axes(_FAKE_REGISTRY, "clean_table") == _ModelA_AXES  # ft tag
...
def test_registry_returns_none_when_no_labels():
    ...
    assert _resolve_action_axes([], "model-a") is None  # empty registry (env override)
