...
# Hardcoded ModelA-FP3 WS URL used by warmup_modal_model_a and GT5 without NT_INFERENCE_URL
# so discovery-based routing can be exercised. _MODEL_ENDPOINTS is gone as of brief-218.
_ModelA_FP3_WS_URL = "wss://removed-endpoint.invalid/stream"
...
@pytest.fixture(scope="session")
def warmup_modal_model_a() -> None:
    """Poll /health on the ModelA-FP3 endpoint until loaded.

    Uses _ModelA_FP3_WS_URL directly — GT5 tests discovery-based routing so
    NT_INFERENCE_URL is not set here. Skips GT5 (via pytest.skip) if the
    endpoint doesn't report loaded=True within 180s.
    Deploy first: modal deploy modal_tunnel_modela.py
    """
    if not os.environ.get("NT_API_KEY"):
        return  # api_key fixture will skip the test; don't block here

    health_url = _health_url_from_inference_url(_ModelA_FP3_WS_URL)
    deadline = time.monotonic() + 180.0
    t0 = time.monotonic()
    last_err: Exception | None = None
    last_body: str = ""
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if body.get("status") == "ok" and body.get("loaded") is True:
                    elapsed = time.monotonic() - t0
                    print(
                        f"\n[warmup_modal_model_a] /health ok+loaded after {elapsed:.1f}s "
                        f"(url={health_url})",
                        flush=True,
                    )
                    return
    ...
    elapsed = time.monotonic() - t0
    pytest.skip(
        f"warmup_modal_model_a: /health did not report loaded=true within 180s "
        f"(elapsed={elapsed:.1f}s, last_body={last_body!r}, last_err={last_err!r}) — "
        f"deploy the endpoint first: modal deploy modal_tunnel_modela.py"
    )


def test_gt5_model_a_model_selection(
    api_key: str, warmup_modal_model_a, monkeypatch
) -> None:
    """A developer who needs ModelA-FP3 specifically can opt in via model='model-a' and gets back chunks shaped for that model.

    Developer constructs newt.Robot(..., model="model-a") with their API key and two
    callables. They call robot.run("clean_table"). Chunks come back shaped for ModelA-FP3's
    wire format: each timestep is [x, y, z, qw, qx, qy, qz, gripper] (8 dims). The
    ...
    ModelA-FP3 wire requirement (brief-205-cont-b): read_state() must return depth_maps +
    ...
    Assert: read_state invoked >= 1 time; execute invoked >= 1 time with ndarray chunk;
    each chunk timestep is 8-dimensional (verifying ModelA-FP3 wire format, not pi05-aloha's);
    result.stop_reason in valid set; on ModelA-FP3 baseline today (no learned stop token),
    expect "max_duration". Wall time <= max_duration + 5s overhead (endpoint is warm;
    warmup_modal_model_a fixture ensures that before the test body runs).
    ...
    # Clear NT_INFERENCE_URL so model="model-a" resolves via registry, not the override.
    ...
    _ModelA_CAMERA_KEYS = ["right-wrist-camera", "surrounding1", "surrounding2"]
    ...
    def read_state() -> dict:
        ...
        return {
            ...
            "depth_maps": {cam: np.zeros((240, 320), dtype=np.float32) for cam in _ModelA_CAMERA_KEYS},
            "intrinsics":  {cam: np.eye(3, dtype=np.float32) for cam in _ModelA_CAMERA_KEYS},
            "extrinsics":  {cam: np.eye(4, dtype=np.float32) for cam in _ModelA_CAMERA_KEYS},
        }

    robot = newt.Robot(
        api_key=api_key,
        read_state=read_state,
        execute=lambda c: execute_calls.append(c),
        model="model-a",
    )
    ...
    assert read_state_calls >= 1, "read_state() must be called at least once"
    assert len(execute_calls) >= 1, "execute() must be called at least once (>= 1 action chunk)"
    for i, chunk in enumerate(execute_calls):
        assert isinstance(chunk, np.ndarray), f"chunk[{i}] must be ndarray"
        assert chunk.shape[-1] == 8, (
            f"chunk[{i}] timestep must be 8D [x,y,z,qw,qx,qy,qz,gripper], got {chunk.shape} — "
            f"rot6d on the wire is the failure mode; server must convert to quaternion before emit"
        )
    assert result.stop_reason in _VALID_STOP_REASONS, (
        f"stop_reason {result.stop_reason!r} not in {_VALID_STOP_REASONS}"
    )
    assert elapsed <= max_duration + 5.0, (
        f"run() took {elapsed:.1f}s, expected <= {max_duration + 5.0}s "
        f"(endpoint warm per warmup_modal_model_a)"
    )
