...
def warmup_modal_nt0_fp3() -> None:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if body.get("status") == "ok" and body.get("loaded") is True:
                    elapsed = time.monotonic() - t0
                    print(
                        f"\n[warmup_modal_nt0_fp3] /health ok+loaded after {elapsed:.1f}s "
                        f"(url={health_url})",
                        flush=True,
                    )
                    return
    ...
    elapsed = time.monotonic() - t0
    pytest.skip(
        f"warmup_modal_nt0_fp3: /health did not report loaded=true within 180s "
        f"(elapsed={elapsed:.1f}s, last_body={last_body!r}, last_err={last_err!r}) — "
        f"deploy the endpoint first: modal deploy modal_tunnel_nt0.py"
    )


def test_gt5_nt0_fp3_model_selection(
    api_key: str, warmup_modal_nt0_fp3, monkeypatch
) -> None:
    """A developer who needs NT0-FP3 specifically can opt in via model='nt0-fp3' and gets back chunks shaped for that model.

    Developer constructs newt.Robot(..., model="nt0-fp3") with their API key and two
    callables. They call robot.run("clean_table"). Chunks come back shaped for NT0-FP3's
    ...
    # Clear NT_INFERENCE_URL so model="nt0-fp3" resolves via registry, not the override.
    ...
    assert elapsed <= max_duration + 5.0, (
        ...
        f"(endpoint warm per warmup_modal_nt0_fp3)"
    )
