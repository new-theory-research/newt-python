...
def test_gt5_model_a_model_selection(
    ...
    ModelA-FP3 geometry contract (brief-258b — warn, don't block): ModelA-FP3 declares
    ...
    ModelA-FP3 wire format, not pi05-aloha's); result.stop_reason in valid set; on ModelA-FP3
    baseline today (no learned stop token), expect "max_duration". Wall time <=
    max_duration + 5s overhead (endpoint is warm; warmup_modal_model_a fixture ensures
    ...
    assert _deg, (
        "running ModelA-FP3 with no depth_maps/intrinsics/extrinsics must emit a "
