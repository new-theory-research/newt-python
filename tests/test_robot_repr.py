...
Why these matter:
  The public docs' milestone-1 finish line shows an exact string:
      model-a · contract received · (50,8) · 8 labeled axes
...
def test_repr_full_contract_matches_docs_receipt():
    ...
    expected = "model-a · contract received · (50,8) · 8 labeled axes"
    assert repr(robot) == expected
    assert str(robot) == expected  # AC 3: __str__ == __repr__


def test_repr_uses_tag_over_uid():
    """Display name is the first tag (model-a), not the raw UID (ft_base_modelafp3).
    ...
    """
    robot = _make_robot(_FULL_REGISTRY)
    assert "model-a" in repr(robot)
    assert "ft_base_modelafp3" not in repr(robot)


def test_repr_explicit_model_arg():
    """Robot(model='model-a') resolves the same display string as Robot().
    ...
    robot_explicit = _make_robot(_FULL_REGISTRY, model="model-a")
...
def test_repr_contract_missing_axes_degrades():
    ...
    result = repr(robot)
    assert "contract received" not in result
    assert "model-a" in result  # still shows the model name
    ...
    registry_no_shape = [
        {
            ...
            "tags": ["model-a"],
            "endpoint": "wss://example/stream",
            "contract": {"action_axes": _ModelA_AXES},  # shape missing
        }
    ]
