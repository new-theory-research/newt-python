...
def _fake_response(model="model-a-pour", latency_ms=42.0):
...
def test_happy_path_prints_model_latency_shape_and_framing(monkeypatch):
    ...
    """
    resp = _fake_response(model="model-a-pour", latency_ms=37.0)
    code, out, err, _ = _run(["model-a-pour"], monkeypatch, infer_return=resp)

    assert code == 0, f"expected exit 0; stderr={err!r}"
    assert err == ""
    assert "model-a-pour" in out, "resolved model must appear"
...
def test_json_mirror_is_structured_and_prose_free(monkeypatch):
    ...
    resp = _fake_response(model="model-a-pour", latency_ms=42.0)
    code, out, err, _ = _run(["model-a-pour", "--json"], monkeypatch, infer_return=resp)

    assert code == 0, f"stderr={err!r}"
    data = json.loads(out)
    assert data["tag"] == "model-a-pour"
    assert data["model"] == "model-a-pour"
...
def test_model_not_found_renders_message(monkeypatch):
    ...
    exc = ModelNotFoundError(model="typo-tag", known=["model-a", "model-a-pour"])
...
def test_base_not_deployable_renders_message(monkeypatch):
    ...
    exc = BaseNotDeployableError(
        ...
        message="'model-a' is a base — run one of its fine-tunes: model-a-pour.",
        context={"model": "model-a", "fine_tunes": ["model-a-pour"]},
    )
    code, out, err, _ = _run(["model-a"], monkeypatch, construct_raises=exc)
    assert code == 1
    assert err.startswith("newt: ")
    assert "base" in err.lower()
    assert "model-a-pour" in err, "the deployable fine-tunes must survive to the user"
...
def test_verifier_retry_with_small_gap_still_renders_honest_note(monkeypatch):
    ...
    chunk = np.zeros((50, 8), dtype=np.float32)
    resp = InferenceResponse(
        action_chunk=chunk, axes=list(_AXES), latency_ms=180.0, model="model-a-pour",
