...
def _depth_warning_payload() -> dict:
    """The shape serve_modela._build_warning_from_catalog('degradation.missing_depth_fields')
...
def test_missing_depth_fields_names_fields_and_impact():
    ...
    with warnings.catch_warnings(record=True) as rec:
        ...
        _maybe_warn_degradation(frame, "model-a")
...
def test_no_warning_on_clean_frame():
    ...
    with warnings.catch_warnings(record=True) as rec:
        warnings.simplefilter("always")
        _maybe_warn_degradation({"type": "action"}, "model-a")
        _maybe_warn_mid_session({"type": "action"}, "model-a")
...
def test_mid_session_warns_with_frame_index_and_impact():
    ...
    with warnings.catch_warnings(record=True) as rec:
        warnings.simplefilter("always")
        _maybe_warn_mid_session(frame, "model-a")
    msgs = [str(w.message) for w in rec if issubclass(w.category, DegradationWarning)]
    assert len(msgs) == 1
